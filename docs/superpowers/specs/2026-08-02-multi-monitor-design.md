...
**Not `TabWidget`:** an earlier version of this design planned one `Tab` per connected monitor, but Slint's compiler explicitly rejects a data-driven number of tabs — `if`/`for` inside a `TabWidget` produces `"dynamic tabs ('if' or 'for') are currently not supported"` (confirmed against this project's installed `slint` version), and individual `Tab`s can't be shown/hidden dynamically either (`"The property 'visible' cannot be set for Tabs inside a TabWidget"`). Since the number of connected monitors is a runtime fact (varies by hardware, can change via hot-plug), a fixed compile-time set of tabs isn't viable.

Instead: a `ComboBox` (the same widget this project already uses for the interval-unit selector) lists connected monitors by a human-readable label (e.g. `"Monitor 1 (LVDS-1)"`), and the existing single form (folder picker, interval, preview image, countdown, pause/change-now/save) stays as one instance, reused — its bound fields are repopulated whenever the selected monitor changes, the same way `refresh_state`'s 1-second timer already repopulates fields from disk today. Only one monitor's settings are visible/editable at a time; switching the dropdown is instantaneous (no daemon round-trip, just re-reading already-loaded `Config`/`State` data for the newly-selected UUID). `gui/src/main.rs` reads all monitors' config/state up front, and the `choose-folder`/`toggle-pause`/`save` callbacks operate on whichever `MonitorConfig` entry corresponds to the currently-selected dropdown index.

**`change-now` needs to target one monitor, not all of them** — clicking it should only force an immediate rotation on the *currently selected* monitor, consistent with every other control now being scoped to one monitor at a time. The shared `change_now_request` file's *content* becomes the target monitor's UUID (previously just a timestamp, whose value the daemon never actually read — only the file's modification was the signal). `wallpaper_core::config::change_now_request_path()` itself is unchanged; only what gets written to it changes. The daemon's handling (see "Daemon Changes" below) reads that UUID after the file-changed signal fires and applies it to that one monitor only.
